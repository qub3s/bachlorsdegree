...
/** AI Player Class that extends AdvancedPlayer */
public class AIPlayer extends AdvancedPlayer{
    ...
    private Board board;
    /** tile generator for the board */
    private TileGenerator generator;
    /**gameconfiguaration */
    private AdvancedGameConfiguration conf;
    /** Tile Placement of the Last Tile */
    ...
    /** Random number Gernerator for the randomAiMove */
    private Random rand = new Random();

    /** Makes the random moves for the AIPlayer 
    * @param currenttile The Currenttile that gets placed
    * @param tilelist the list of dummytiles, that gets placed
    * @param b the obsever board the moves are played on */
    public TilePlacement randomAiMove(AdvancedTile currenttile, ArrayList<DummyTile> tilelist,Board b){

        TilePlacement tp = tilelist.get(rand.nextInt(tilelist.size())).getTilePlacement();

        while(!b.placementIsViable(currenttile,tp,id)){
            if(tp.getRotation() == 6){
                System.out.println(tp.getColumn());
                throw new RuntimeException("lol");     
            }
            tp = new TilePlacement(tp.getRow(),tp.getColumn(),tp.getRotation()+1);
        }
    ...
    */
    ...
    /** Checks if this Player is an AI PLayer */
    public void setPLayerasAIPlayer(){
        isAIPlayer = true;
        return;
    }
